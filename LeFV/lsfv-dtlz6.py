...
def g_function(X_m):
    epsilon = 1e-10  # 防止零或负数输入导致的问题
    return sum((max(x, epsilon)**0.1) for x in X_m)

def theta(X):
    return [(np.pi / 2) * (x - 0.5) / 0.5 for x in X]  # 基本的线性变换
...
def objective_1(X, num_objectives=3):
    ...
    theta_vals = theta(X[1:num_objectives-1])
    return (1 + g) * np.cos(X[0] * np.pi / 2) * np.prod(np.cos(theta_vals))

def objective_2(X, num_objectives=3):
    assert len(X) >= num_objectives, "Length of decision vector should be at least equal to number of objectives"
    g = g_function(X[num_objectives-1:])
    theta_vals = theta(X[1:num_objectives-1])
    return (1 + g) * np.cos(X[0] * np.pi / 2) * np.sin(theta_vals[0])
...
def objective_3(X, num_objectives=3):
    ...
    theta_vals = theta(X[1:num_objectives-1])
    if len(theta_vals) > 1:
        return (1 + g) * np.sin(X[0] * np.pi / 2) * np.prod(np.sin(theta_vals))
    else:
