...
def DTLZ1(x, M):
    ...
    f = [(1 + g) * np.prod(x[:i]) * (1 - x[i]) if i != 0 else (1 + g) * (1 - x[0]) for i in range(M)]
    return f

def evaluate_population(population, M):
    objectives = [DTLZ1(individual, M) for individual in population]
    return list(zip(population, objectives))
...
def calculate_reference_point(objectives):
    ...
    for i in range(num_objectives):
        if i == 0:
            ...
            reference_point.append(max_value + 0.001)
        elif i == 1:
            min_value = max(ind[i] for ind in objectives)
            reference_point.append(min_value + 0.001)
        elif i == 2:
            min_value = max(ind[i] for ind in objectives)
            reference_point.append(min_value + 0.001)
...
def main():
    ...
    population = evaluate_population(raw_population, num_objectives)
    ...
    for gen in range(num_generations):
        ...
        new_population = evaluate_population(new_raw_population, num_objectives)
        if len(population[0]) == 100:
            population = evaluate_population(population, num_objectives)
