...
func main() {
	...
	if isGen {
		if isGenerateForExeFile {
			...
			switch os {
			case "windows":
				build.Exebat_Windows_GenerateBatFile(shardNum, modID, dataRootDir)
			case "darwin":
				build.Exebat_MacOS_GenerateShellFile(shardNum, modID, dataRootDir)
			case "linux":
				build.Exebat_Linux_GenerateShellFile(shardNum, modID, dataRootDir)
			}
		} else {
			// Without determining the operating system.
			// Generate a .bat file or .sh file for running `go run`.
			build.GenerateBatFile(shardNum, modID, dataRootDir)
			build.GenerateShellFile(shardNum, modID, dataRootDir)
		}
	...
	if isSupervisor {
		build.BuildSupervisor(uint64(shardNum), uint64(modID))
	} else {
		build.BuildNewPbftNode(uint64(nodeID), uint64(shardID), uint64(shardNum), uint64(modID))
	}
}
