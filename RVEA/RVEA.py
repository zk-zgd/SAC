...
def dominates(obj1, obj2):
    ...
    for i in range(len(obj1)):
        if obj1[i] > obj2[i]:
            ...
            sum_less += 1
    return sum_less > 0
def plot_pareto_front(objs, t, show=False):
    """
    Plot the current Pareto front for a 2D or 3D objective space.

    :param objs: Numpy array of objective values.
    :param t: Current generation number.
    :param show: Whether to display the plot window.
    """
    plt.figure(figsize=(10, 8))
    if objs.shape[1] == 2:
        plt.scatter(objs[:, 0], objs[:, 1], c='blue', marker='o', label=f'Generation {t}')
        ...
        ax.scatter(objs[:, 0], objs[:, 1], objs[:, 2], c='red', marker='o', label=f'Generation {t}')
        ax.set_xlabel('Objective 1')
        ax.set_ylabel('Objective 2')
        ax.set_zlabel('Objective 3')
        ax.legend()
        ax.view_init(elev=30, azim=30)  # Adjust the view angle for better visualization
        plt.title('Pareto Front in 3D')

    plt.grid(True)
    plt.savefig(f'pareto_front_gen_{t}.png')  # Save the plot as a PNG file
    if show:
        plt.show()
    plt.close()
...
def main(npop, iter, lb, ub, nobj, eta_c=30, eta_m=20, alpha=2, fr=0.1):
    ...
    for t in range(iter):
        ...
        # 动态更新参考点
        ref_point = np.max(objs, axis=0) + 0.1  # 计算新的参考点
        print("ref_point:", ref_point)
        hv = pg.hypervolume(objs)  # 创建超体积对象
        hv_value = hv.compute(ref_point)  # 计算当前超体积
    ...
    # Final visualization at the end of all generations
    plot_pareto_front(objs, iter, show=True)
    
    # 绘制 HV 曲线
    plt.plot(hv_values)
    plt.xlabel('Iteration')
    plt.ylabel('HV')
    plt.title('Hyper-volume Indicator Over Iterations')
    plt.show()

if __name__ == '__main__':
    main(100, 921, np.array([0] * 7), np.array([1] * 7), 3)
