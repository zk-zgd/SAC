...
N = 200  # 节点数
S = 15    # 分片个数
shard_efficiencies = [529, 666, 221, 851, 797, 745, 621, 456, 592, 664, 431, 336, 547, 621, 445]  # 每个分片的效率
node_in_shard = [13, 14, 13, 15, 12, 14, 14, 13, 14, 14, 14, 15, 11, 14, 10]
...
# 从JSON文件加载节点信息
def load_node_info(file_path="config\\NodeInfo20015.json", encoding='utf-8'):
...
def initialize_population(pop_size, num_nodes, num_shards):
    ...
    for _ in range(pop_size):
        while True:
            ...
            if all(len(nodes) >= 10 for nodes in nodes_per_shard.values()):
                break
...
def save_data_to_csv(hv_values, throughput_values, delay_values, security_values, filename="simulation_results_spea2.csv"):
    ...
    directory = "results-new-ref\\20015"
