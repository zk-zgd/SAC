...
# 定义适应度和个体
# 权重设置为 (1.0, -1.0, -1.0) 表示最大化吞吐量，最小化延迟和安全性
...
# 从JSON文件加载节点信息
def load_node_info(file_path="E:\\mychain\\moa\\ZDT_MOA\\MaOEA\\NodeInfo1.json", encoding='utf-8'):
...
def get_node_coordinates(node_info):
    ...
    for k, v in node_info.items():
        shard_id = int(k[:2])
        node_id = int(k[2:])
        node_global_id = (shard_id - 1) * 40 + node_id  # 假设每个分片有20个节点
...
def get_node_efficiencies(node_shard_history, shard_efficiencies):
    ...
    for shard_id, node_list in node_shard_history.items():
        for node_id in node_list:
            global_node_id = (shard_id - 1) * 40 + node_id  # 映射到全局节点ID
...
def initialize_population(pop_size, num_nodes, num_shards, min_per_shard=12):
    population = []
    for _ in range(pop_size):
        while True:
            ...
            if all(len(nodes) >= min_per_shard for nodes in nodes_per_shard.values()):
                break
...
def main():
    ...
    # 初始化种群
    population = initialize_population(pop_size, num_nodes, num_shards, min_per_shard=12)
