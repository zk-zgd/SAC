...
node_in_shard = [6,7,8,6,8,5,6,7,5,8,8,6,7,8,5]
...
# 从JSON文件加载节点信息
def load_node_info(file_path="E:\\mychain\\moa\\ZDT_MOA\\MaOEA\\NodeInfo10015.json", encoding='utf-8'):
...
def throughput(individual):
    ...
    nodes_per_shard = get_nodes_per_shard(individual, S)
    efficiencies = np.array([sum(node_efficiencies[node] for node in nodes) / len(nodes) if nodes else 0 for nodes in nodes_per_shard.values()])
    return -np.sum(efficiencies)  # 返回负值以最小化
...
def initialize_population(pop_size, num_nodes, num_shards):
    ...
    for _ in range(pop_size):
        while True:
            ...
            if all(len(nodes) >= 5 for nodes in nodes_per_shard.values()):
                break
...
def save_data_to_csv(hv_values, throughput_values, delay_values, security_values, filename="simulation_results_hype.csv"):
    ...
    directory = "E:\\mychain\\moa\\ZDT_MOA\\results-new-ref\\15"
...
#################################################################################################
    ...
    # plt.plot(generations, hypervolumes, label='Hypervolume')
